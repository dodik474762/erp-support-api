...
export class VendorCategoryService {
    constructor(
        ...
        private ptRepo: Repository<VendorCategory>,
        private dataSource: DataSource,
      ) {}
    
      async getAll(): Promise<any> {
        return this.ptRepo
          .createQueryBuilder('vendor_category')
          .select(['vendor_category.*'])
          .where('vendor_category.deleted IS NULL')
          .getRawMany();
      }
      ...
      async get(
        ...
        const data = this.ptRepo
          ...
          .andWhere(
            new Brackets((qb) => {
              qb.where(`vendor_category.type LIKE '%${search}%'`).orWhere(
                `vendor_category.remarks LIKE '%${search}%'`,
              );
            }),
          )
          .andWhere(
            new Brackets((qb) => {
              if (filterdate != '') {
                ...
                if (dataDate.length == 2) {
                  qb.where(
                    `vendor_category.created_at BETWEEN '${dataDate[0]}' AND '${dataDate[1]}'`,
                  );
                }
                if (dataDate.length == 1) {
                  qb.where(
                    `cast(vendor_category.created_at as date) = '${dataDate[0]}'`,
                  );
                }
              }
            }),
          )
          .orderBy(`vendor_category.${order}`, 'DESC')
          .limit(limit)
          .offset((page - 1) * limit);
      ...
      async getDetail(id: string): Promise<any> {
        return this.ptRepo
          ...
          .select(['vendor_category.*'])
          .where('vendor_category.deleted IS NULL')
          .andWhere('vendor_category.id = :id', { id: id })
          .getRawOne();
      }
        ...
        try {
          ...
          result.data =
            insertOrUpdate == 'update'
              ? await queryRunner.manager.update(VendorCategory, paramsData.id, params)
              : await queryRunner.manager.save(VendorCategory, params);
