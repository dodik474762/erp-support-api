    ...
    constructor(
      ...
      async getParent(): Promise<any> {
        return this.departementRepo
          ...
          .where(
            new Brackets((qb) => {
              qb.where('department.department_name IS NULL').orWhere("department.code = ''");
            }),
          )
            ...
            new Brackets((qb) => {
              if (filterdate != '') {
                ...
                if (dataDate.length == 1) {
                  qb.where(`cast(department.created_at as date) = '${dataDate[0]}'`);
                }
              }
            }),
          )
      ...
      async delete(id: string): Promise<any> {
        ...
        try {
          result.data = await this.departementRepo.update(id, { deleted: new Date() });
          result.is_valid = true;
          result.message = 'Success';
        } catch (error) {
