...
@Controller('/api/approval/vendor')
export class VendorController {
  constructor(
    private services: VendorService,
  ...
  index(): any {
    return {
      statusCode: 200,
      message: 'Module Approval Request Vendor',
    };
  }
  ...
  @Post('/submit')
  async submit(@Body('id') id: string, @Req() req: any): Promise<any> {
    const user = req.user;
    const data: any = {
      id: id,
      updated_at: new Date(),
